...
namespace Banquo.Extensions
{
   public partial class User : IWebDriver
   {
      ...
      public DOMElement ForceClear(By by, int msTimeout = Banquo.DefaultTimeout) =>
            WaitForEnabled(by, msTimeout).ForceClear();
      public DOMElement ForceClear(string selector, int msTimeout = Banquo.DefaultTimeout) =>
            ForceClear(ByRouter(selector), msTimeout);
      ...
      public DOMElement Type(By by, string toType, int typingDelay = 0, int msTimeout = Banquo.DefaultTimeout) =>
          WaitForEnabled(by, msTimeout).Type(toType, typingDelay);

      public DOMElement Type(string selector, string toType, int typingDelay = 0, int msTimeout = Banquo.DefaultTimeout) =>
          Type(ByRouter(selector), toType, typingDelay, msTimeout);

      public DOMElement AppendsField(By by, string toAppend, int msTimeout = Banquo.DefaultTimeout) =>
         AppendsField(by, toAppend, msTimeout);

      public DOMElement AppendsField(string selector, string toAppend, int msTimeout = Banquo.DefaultTimeout) =>
          AppendsField(ByRouter(selector), toAppend, msTimeout);
